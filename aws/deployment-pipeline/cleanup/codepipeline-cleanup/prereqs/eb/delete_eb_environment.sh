...
if (aws elasticbeanstalk describe-environments --application-name ${APPLICATION_NAME} --environment-names ${ENVIRONMENT_NAME} | jq '(.Environments[] | select(.Status != "Terminated"))' | grep -q ${ENVIRONMENT_NAME}); then
