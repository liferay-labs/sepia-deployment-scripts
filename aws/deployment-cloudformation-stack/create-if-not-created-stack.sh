...
for param in $CONFIG_DIR/params/*.json; do
  ...
  if [ $? -eq 0 ]; then
    ...
    cloudformation_command="aws cloudformation create-stack --stack-name $stack_name --template-body file://${CONFIG_DIR}/template/template.json --parameters file://${param} --region $REGION --capabilities CAPABILITY_IAM"
