...
function usage {
    ...
    echo "  -r awsregion     (optional) AWS_DEFAULT_REGION of the stack, if this param is empty, the env variable AWS_REGION will be used instead"
    exit 1
}
...
for param in $CONFIG_DIR/params/*.json; do
  ...
  if [ $? -eq 0 ]; then
    ...
    cloudformation_command="aws cloudformation create-stack --stack-name $stack_name --template-body file://${CONFIG_DIR}/template/template.json --parameters file://${param} --region $REGION --capabilities CAPABILITY_IAM CAPABILITY_NAMED_IAM"
